"""Tests for punt_zspec.coverage, driven by real probcli transcripts.
...
the contract, and an invented fixture agrees with a broken parser. The
hand-built census strings below are the exception and are deliberate — they
are malformed on purpose, to exercise grammar the real tool never emits.
"""
...
from punt_zspec.coverage import CoverageCensus, UnreadableCoverage
...
def test_a_run_that_never_asked_reports_a_failed_check(fixture: str) -> None:
    ...
    assert check.status == CheckStatus.failed
    assert check.detail == "probcli printed no coverage census"
