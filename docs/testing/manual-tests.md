...
| D2 | `z-spec browse` / **Browse** menu entry | a real repo with specs | only the repo's **real** specs (`examples/`, `tutorials/`); each labelled with a readable name, not a truncated path; selecting one shows its content |
