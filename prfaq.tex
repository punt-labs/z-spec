...
{\small\texttt{/z-spec:code2model}} to write a spec for a new system, or
{\small\texttt{/z-spec:code2model}} pointing at your codebase to generate one from existing code.
...
Three commands: (1)~install the plugin, (2)~run \texttt{/z-spec:setup all} to install fuzz and ProB, (3)~choose your workflow. For a new system, run \texttt{/z-spec:code2model} with a natural-language description to write the spec first. For existing code, run \texttt{/z-spec:code2model} pointing at your codebase, then \texttt{/z-spec:partition} to derive test cases or \texttt{/z-spec:test} to find bugs. The LLM drafts the spec; fuzz and ProB check it. No Z notation knowledge is required to start --- the plugin generates idiomatic Z and explains what it means.
\end{faqpair}
...
\textbf{New system with no code yet:} Use \texttt{/z-spec:code2model} with a natural-language description to write the spec first. Iterate with \texttt{/z-spec:check} and \texttt{/z-spec:test} until the spec captures your intended behavior, then generate code and tests from it. This is the highest-value workflow --- bugs are cheapest to fix when no code exists yet.
...
\textbf{Competitive response:} The category is attracting serious capital (\$125M from Tessl alone) and distribution advantages (GitHub/Microsoft, Amazon/AWS). If these players moved toward formal verification, they could integrate existing tools (Dafny, Alloy, or even Z/ProB) faster than 6--12 months. Our differentiation is formal-verification depth (21~commands, three workflows from one specification) in a niche that large players are unlikely to prioritize near-term. The structural moat is thin --- the defensible value is in execution quality and being first to demonstrate the workflow.
\end{faqpair}
...
\item Release v1.0 on the Punt Labs marketplace (already at v0.19.0, shipped end-to-end on PyPI and via the plugin marketplace).
...
Features that deepen the verification chain or extend the core workflows. Not required for the spec-first, test-coverage, or bug-finding loops but significantly increase value. All items marked \textcolor{RiskGreen}{\textbf{shipped}} are already available in v0.19.0.
...
\begin{enumerate}[nosep,leftmargin=2.5em]
  ...
  \featureitem{B-Method support}{\texttt{/z-spec:b-create}, \texttt{/z-spec:b-check}, \texttt{/z-spec:b-animate}, \texttt{/z-spec:b-refine} --- deterministic refinement chain from spec to provably correct code. \textcolor{RiskGreen}{\textbf{Shipped (alpha).}}}\label{feat:b-method}
  ...
  \featureitem{Standalone CLI + MCP}{\texttt{z-spec} on \texttt{PATH} with no Claude Code plugin, installed via \texttt{--no-plugin}. Every deterministic verb the plugin exposes --- \texttt{check}, \texttt{test}, \texttt{animate}, \texttt{model-check}, \texttt{partition}, \texttt{audit}, \texttt{show}, \texttt{browse}, \texttt{pick}, \texttt{report}, \texttt{doctor} --- is reachable from a shell, a script, or an agent that is not the Claude plugin (Codex, Cursor, a plain terminal). \textcolor{RiskGreen}{\textbf{Shipped.}}}\label{feat:standalone}
