...
\begin{enumerate}[nosep,leftmargin=2.5em]
  ...
  \featureitem{Standalone CLI + MCP}{\texttt{z-spec} on \texttt{PATH} with no Claude Code plugin, installed via \texttt{--no-plugin}. Every deterministic capability --- \texttt{check}, \texttt{test}, \texttt{partition}, \texttt{audit}, \texttt{show} --- is reachable from a shell, a script, or an agent that is not the Claude plugin (Codex, Cursor, a plain terminal). \textcolor{RiskGreen}{\textbf{Shipped.}}}\label{feat:standalone}
