...
{\small\texttt{/z-spec:code2model}} with a description instead of a codebase path. The LLM drafts a Z specification with states, invariants, and operations. The developer runs
...
\noindent Then choose a workflow with {\small\texttt{/z-spec:code2model}}: describe a new system in natural language to write its spec first, or point it at your codebase to generate one from existing code.
...
Install the plugin, run \texttt{/z-spec:setup all} to install fuzz and ProB, then choose a workflow. For a new system, run \texttt{/z-spec:code2model} with a natural-language description to write the spec first. For existing code, run \texttt{/z-spec:code2model} pointing at your codebase, then \texttt{/z-spec:partition} to derive test cases or \texttt{/z-spec:test} to find bugs.
\end{faqpair}
...
\textbf{New system with no code yet:} Use \texttt{/z-spec:code2model} with a natural-language description to write the spec first, then generate code and tests once it captures your intended behavior. This is the highest-value workflow --- bugs are cheapest to fix when no code exists yet.
...
\textbf{Internal adoption is real and independently verifiable, but it is not a clean market signal --- three of the largest consumers now mandate the practice.}

\emph{Positive signals:}
\begin{itemize}[nosep]
\item Eight of the sibling projects in the Punt Labs portfolio have reached for Z Spec to model a real subsystem. Counting only real specifications --- excluding z-spec's own dogfood corpus of examples, tutorials, and test fixtures, and excluding \texttt{.tmp/} scratch clones and \texttt{.claude/worktrees/} duplicates in the sibling repositories --- there are 57 specification files across those 8 repositories. The breakdown: biff (10), lux (26, the largest consumer), vox (12), quarry (2), ethos (2), beadle (2), koch-trainer-swift (2), and mcp-proxy (1). Several of these are variant families investigating one underlying subject rather than 57 independent subjects --- lux alone has five subjects (board ordering, display-crash loops, header-toggle reconciliation, hub-display reconciliation, scene-ID namespacing) spread across 15 files, and biff's NATS-relay work spans 3 files under one subject. The distinct-subsystem count is therefore in the low forties, not 57.
\item The Koch Trainer iOS app was specified before implementation (spec-first): ProB validated the state machine before a line of Swift was written. Biff's own committed \texttt{.audit.json} report is a real artifact of the test-derivation workflow, generated by \texttt{/z-spec:audit} against its connection-lifecycle spec. And several of lux's reconciliation specifications carry deliberate ``buggy'' variants alongside the fixed version: a spec that cannot reproduce the defect it guards against is too abstract to trust. So the team builds the failing model first, then confirms the passing one catches what the failing one doesn't. That is bug finding with a falsification test built in, not just a claim that the spec ``covers'' the bug.
\item All three of the largest consumers --- biff, vox, and lux --- began writing Z specifications independently in early March 2026. Four months later, on 2026-07-04, all three repositories made the practice mandatory in one coordinated policy decision, not three separate ones. Roughly two-thirds of the 57 specification files were written after that date, so most of the corpus reflects a requirement rather than open-ended choice. But the four months of voluntary use that preceded it justified making it policy.
...
\textbf{Competitive response:} The category is attracting capital (\$125M from Tessl alone) and distribution advantages (GitHub/Microsoft, Amazon/AWS). If these players moved toward formal verification, they could integrate existing tools (Dafny, Alloy, or even Z/ProB) faster than 6--12 months. Our differentiation is formal-verification depth (21~commands, three workflows from one specification) in a niche that large players are unlikely to prioritize near-term. The structural moat is thin --- the defensible value is in execution quality and being first to demonstrate the workflow.
\end{faqpair}

\begin{faqpair}{What is our next step to validate the vision?}\label{faq:validation}
Internal dogfooding already shows the mechanism works (\faqref{faq:evidence}), including specifications that formally pin down real defects and prove a fix closes them. That validates feasibility and utility within one organization's own engineering practice. It does not validate market demand --- adoption in the three largest consumers preceded a mandate that now requires it, and every one of these specifications was written by engineers or dispatched sub-agents with direct access to the tool's creators.
...
\item Release v1.0 on the Punt Labs marketplace (already at v0.20.2, shipped end-to-end on PyPI and via the plugin marketplace).
\item Track installs, active usage (\texttt{check}, \texttt{code2model}, \texttt{partition}, and \texttt{test} invocations), and GitHub stars over 90~days. Track which workflow each user enters through.
...
The plugin does not yet guide this decomposition. Multi-spec coordination (specifications that reference each other across modules) is on the roadmap (\faqref{faq:timeline}) but not shipped. Until then, the practical ceiling is single-module specifications with bounded state --- matching the corpus demonstrated so far (\faqref{faq:evidence}): single-subsystem state machines and reconciliation logic, not whole-service models.
...
Z Spec is at v0.20.2 with 21 commands shipped, on PyPI as \texttt{punt-z-spec} and on the Punt Labs plugin marketplace. Development priorities in delivery order:
...
\textbf{AI coding assistants reached mainstream adoption} (\faqref{faq:tam}). Yet only 29\% trust the accuracy of AI-generated output~\cite{stackoverflow2025survey}. This gap --- widespread use with low trust --- created the ``vibe coding'' problem at scale: developers generating code from loose prompts with no systematic verification beyond testing.
...
\midrule
Value      & \textcolor{RiskAmber}{Medium} & Three problems are real and growing: new systems built without blueprints, existing code with shallow test coverage, and running systems with latent defects. Internal usage across 8 sibling repositories (57 specification files, \faqref{faq:evidence}) shows the mechanism delivers real value --- including formally proving a concurrency fix closes the exact defect a weaker design would not, with a falsification-test variant confirming it --- but external developer adoption, and which workflow drives it, remains unproven. The math-notation barrier may limit adoption to developers who already value correctness. \\
...
Feasibility & \textcolor{RiskAmber}{Medium} & The product exists at v0.20.2 with 21~commands, on PyPI and the plugin marketplace. fuzz and ProB are proven tools, so the core mathematics is not in question, and 57 real specification files across 8 sibling repositories (\faqref{faq:evidence}) show the pipeline holds up across genuinely different problem domains, not just the tool's own examples --- including deliberate falsification-test variants that confirm a spec actually catches the defect it claims to model. Two feasibility concerns are still live: (a) LLM spec quality (\faqref{faq:tech-risks}) --- a well-typed spec can still describe the wrong system; and (b) performance on a single-user machine (\faqref{faq:scaling}) --- the engine scales, but RAM and CPU on a typical developer laptop bind before ProB does, and the hardware-to-scale equation at each specification size is not yet mapped. Stability work is ongoing; the deployment story beyond a personal laptop has not been evaluated. \\
...
\begin{enumerate}[nosep,leftmargin=2.5em]
  \featureitem{Spec generation}{\texttt{/z-spec:code2model} --- the LLM drafts Z specifications from a natural-language description (spec-first development) or from codebase analysis (test derivation and bug finding for existing systems)}\label{feat:code2model}
...
Features that deepen the verification chain or extend the core workflows. Not required for the spec-first, test-coverage, or bug-finding loops but increase value. All items marked \textcolor{RiskGreen}{\textbf{shipped}} are already available in v0.20.2.

\begin{enumerate}[nosep,leftmargin=2.5em]
  \featureitem{Code generation from specs}{\texttt{/z-spec:model2code} --- generates implementation code and tests from a specification, completing the spec-first workflow. \textcolor{RiskGreen}{\textbf{Shipped.}}}\label{feat:model2code}
  \featureitem{Automated audit}{\texttt{/z-spec:audit} --- runs the full model-check-test-report chain in one command for rapid bug finding across existing systems. \textcolor{RiskGreen}{\textbf{Shipped.}}}\label{feat:audit}
  \featureitem{Lean~4 proof obligations}{\texttt{/z-spec:prove} --- machine-checked proofs that invariants hold universally, not just for tested inputs. \textcolor{RiskGreen}{\textbf{Shipped.}}}\label{feat:prove}
  \featureitem{Property-based test oracle}{\texttt{/z-spec:oracle} --- uses Lean~4 model as ground truth for randomized testing. \textcolor{RiskGreen}{\textbf{Shipped.}}}\label{feat:oracle}
  \featureitem{Refinement verification}{\texttt{/z-spec:refine} --- verifies that implementation code refines the specification via abstraction function commutativity. \textcolor{RiskGreen}{\textbf{Shipped.}}}\label{feat:refine}
  \featureitem{B-Method support}{the four \texttt{/z-spec:b-*} commands (\texttt{b-create}, \texttt{b-check}, \texttt{b-animate}, \texttt{b-refine}) --- deterministic refinement chain from spec to provably correct code. \textcolor{RiskGreen}{\textbf{Shipped (alpha).}}}\label{feat:b-method}
  \featureitem{Spec elaboration}{\texttt{/z-spec:elaborate} --- enhances a bare spec with narrative from adjacent design documentation, so the same file reads as both a mathematical model and a design brief. \textcolor{RiskGreen}{\textbf{Shipped.}}}\label{feat:elaborate}
  \featureitem{Standalone CLI + MCP}{\texttt{z-spec} on \texttt{PATH} with no Claude Code plugin, installed via \texttt{--no-plugin}. All eleven deterministic verbs the plugin exposes --- from \texttt{check} and \texttt{test} through \texttt{report} and \texttt{doctor} --- are reachable from a shell, a script, or an agent that is not the Claude plugin (Codex, Cursor, a plain terminal). \textcolor{RiskGreen}{\textbf{Shipped.}}}\label{feat:standalone}
  \featureitem{Tutor mode}{adaptive learning that teaches Z notation as the developer uses the tool, reducing the adoption barrier. \textcolor{RiskAmber}{\textbf{Open}} (\texttt{z-spec-p4a}, \texttt{z-spec-yfv}).}\label{feat:tutor}
\end{enumerate}
