...
# /z-spec-dev:refine-dev - Data Refinement Verification
...
> Lean 4 is not installed. Run `/z-spec-dev:setup-dev lean` to install
...
should also exist (from `/z-spec-dev:model2code-dev` or hand-written).
...
from the partition table (if `/z-spec-dev:partition-dev` has been run)
...
Use partition data from `/z-spec-dev:partition-dev` if available.
...
create the project structure (same as `/z-spec-dev:prove-dev`).
...
Follow the same batch discharge process as `/z-spec-dev:prove-dev`:
...
- "For stronger coverage, run `/z-spec-dev:partition-dev` to derive
...
| Error | Response |
|-------|----------|
| Specification not found | "No Z specification found. Specify path or create one with `/z-spec-dev:code2model-dev`." |
...
| Implementation not found | "Could not locate implementation code for {StateName}. Specify the file path: `/z-spec-dev:refine-dev spec.tex typescript --impl src/account.ts`" |
| Language not detected | "Could not detect target language. Specify explicitly: `/z-spec-dev:refine-dev spec.tex typescript`" |
| Lean not installed (with --lean) | "Lean 4 is not installed. Run `/z-spec-dev:setup-dev lean` to install via elan, or omit `--lean`." |
...
Running `/z-spec-dev:refine-dev docs/account.tex typescript --generate-abstraction`
produces:
...
4. For systematic test inputs, run `/z-spec-dev:partition-dev docs/account.tex`
   and incorporate the partition values.
```

## Integration with Other Commands

### With /z-spec-dev:check-dev

Always type-check the specification before running refinement.
`/z-spec-dev:check-dev` validates Z syntax; `/z-spec-dev:refine-dev` verifies
that the implementation correctly refines the spec.

### With /z-spec-dev:model2code-dev

`/z-spec-dev:model2code-dev` generates implementation code from the spec.
`/z-spec-dev:refine-dev` then verifies that the generated (or modified)
code still correctly refines the spec. They form a natural
workflow: generate, customize, then verify.

### With /z-spec-dev:prove-dev

`/z-spec-dev:prove-dev` generates invariant preservation proofs about
the specification itself. `/z-spec-dev:refine-dev --lean` generates
...
### With /z-spec-dev:partition-dev

`/z-spec-dev:partition-dev` derives systematic test inputs via TTF tactics.
...
### With /z-spec-dev:test-dev (oracle testing)

`/z-spec-dev:test-dev` uses random operation sequences to check the
implementation against the spec via ProB animation. `/z-spec-dev:refine-dev`
