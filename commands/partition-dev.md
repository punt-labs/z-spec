...
allowed-tools: mcp__plugin_z-spec-dev_zspec__save_partition_report, mcp__plugin_z-spec-dev_zspec__show_z_spec, Read, Glob, Grep, Write
---

# /z-spec-dev:partition-dev - Derive Test Cases from Z Specification
...
should already exist and have been type-checked (via `/z-spec-dev:check-dev`).
...
Call `mcp__plugin_z-spec-dev_zspec__save_partition_report` with `file` (the spec
...
After running `/z-spec-dev:partition-dev`, the user can run `/z-spec-dev:audit-dev` to check
...
`/z-spec-dev:model2code-dev` generates starter tests using intuitive enumeration.
`/z-spec-dev:partition-dev` generates systematic tests using formal partitioning.
