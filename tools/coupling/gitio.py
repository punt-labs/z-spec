...
class Diff:
    ...
    renamed file can inherit its predecessor's baseline entry.
    """
...
class GitRepo:
    ...
    def diff(self, base: str) -> Diff:
        ...
        clean checkout (CI), and inclusive of a developer's tracked pre-commit
        edits locally. A git failure raises ``GitError`` -- never a silent
        empty set that would let a regression slip past the scope.
        """
