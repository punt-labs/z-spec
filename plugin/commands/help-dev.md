...
start; `/z-spec-dev:code2model-dev` will ask follow-up questions if the
description is ambiguous.

### 2. Draft the specification

Run `/z-spec-dev:code2model-dev <their description>`. This writes a first `.tex`
spec to `docs/` with given sets, a state schema, an `Init` schema, and a
few operations — the raw material, not a finished spec.

### 3. Type-check it

Run `/z-spec-dev:check-dev <the file>`. fuzz will very likely report errors on a
first draft — that is normal, not a failure. Fix each one and re-run until
`/z-spec-dev:check-dev` reports `OK`. This loop, not the first draft, is where the
spec actually gets built.

### 4. Model-check it

Run `/z-spec-dev:test-dev <the file>`. probcli animates the spec and explores its
...
Once `/z-spec-dev:test-dev` passes clean, tell the user plainly: they now have a
...
- `/z-spec-dev:partition-dev` — derive test cases from the spec
- `/z-spec-dev:model2code-dev` — generate code and tests from it
- `/z-spec-dev:audit-dev` — check existing test coverage against it
- `/z-spec-dev:prove-dev` — generate proof obligations, if the invariants matter enough to prove

## Learning Z Notation Itself

The path above teaches the *toolchain* — it assumes just enough Z to read
what `/z-spec-dev:code2model-dev` writes. To learn the *notation* — given sets,
...
| Command | Description |
|---------|--------------|
| `/z-spec-dev:enable-dev` | Turn z-spec on in this repository |
| `/z-spec-dev:disable-dev` | Turn z-spec off in this repository |
...
| `/z-spec-dev:help-dev` | Run this getting-started flow |
...
Run `/z-spec-dev:doctor-dev` to check what's already installed, or `/z-spec-dev:setup-dev`
