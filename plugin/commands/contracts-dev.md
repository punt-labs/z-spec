...
functions in the target language from Z schemas. Unlike `/z-spec-dev:model2code-dev`
...
has not been checked, suggest running `/z-spec-dev:check-dev` first but
 ...
 *   assertDepositPre(state, amount);
 *
 * Generated by /z-spec-dev:contracts-dev
 */
```
...
Also import the model types generated by `/z-spec-dev:model2code-dev`
...
Use the same type mappings as `/z-spec-dev:model2code-dev`:
...
| fuzz not found | "fuzz not installed. Skipping type-check. Contracts will be generated but may reference invalid types. Run `/z-spec-dev:setup-dev` to install fuzz." |
...
Running `/z-spec-dev:contracts-dev docs/counter.tex typescript --wrap`
...
generated by `/z-spec-dev:model2code-dev`. Run `model2code` first to
...
### With /z-spec-dev:audit-dev

After generating contracts, run `/z-spec-dev:audit-dev` to verify that
