...
> The Lean 4 project is missing or incomplete. Run `/z-spec-dev:prove-dev`
> first to generate the Lean model, then run `/z-spec-dev:oracle-dev`.
...
> Consider re-running `/z-spec-dev:prove-dev` to regenerate.
...
- If no Lean project: "Run `/z-spec-dev:prove-dev` first to generate the
  Lean model."
...
| Error | Response |
|-------|----------|
| Lean not installed | "Lean 4 is not installed. Run `/z-spec-dev:setup-dev lean` to install via elan." |
| Lake not installed | "Lake not found. It should be included with Lean 4. Run `elan toolchain install leanprover/lean4:v4.16.0`." |
| No proofs/ directory | "No Lean project found. Run `/z-spec-dev:prove-dev` first to generate the Lean model." |
...
| Lean model out of sync | "Warning: Lean model may not match the current specification. Consider re-running `/z-spec-dev:prove-dev`." |
...
And an existing Lean project from `/z-spec-dev:prove-dev`, the command
generates:
...
`/z-spec-dev:prove-dev` generates the Lean model and proves invariant
preservation. `/z-spec-dev:oracle-dev` turns that proven model into an
...
`/z-spec-dev:partition-dev` derives structural test cases from the
specification using TTF tactics. `/z-spec-dev:oracle-dev` generates
...
`/z-spec-dev:oracle-dev` generates tests that verify the implementation
...
`/z-spec-dev:check-dev` validates Z syntax; `/z-spec-dev:oracle-dev` validates
