...
\texttt{spInactive} and \texttt{spEnded} both describe a machine with no
session running, and they are not the same state. Before
\texttt{SessionStart} there is no session, so there is no identifier;
after \texttt{EndSession} there is a session that has finished, and it
keeps its identifier. That is what the identity invariant below says, and
it is why \texttt{EndSession} releases every resource the session held ---
subagents, context, pending work --- while retaining the one thing that is
not a resource: the name of the session that ended. A terminal state that
has forgotten which session terminated records nothing, and the
\texttt{SessionEnd} hook, which fires with the session id, would have
nothing to fire about.
...
All subagents must have stopped. Every resource the session held is
released; the session identifier is kept, because it is the record of
which session ended and the argument the \texttt{SessionEnd} hook fires
with.
...
The declared halt. Once the session has ended nothing further changes,
and this schema says so: it is enabled exactly in \texttt{spEnded} and
leaves the state alone.

Stating the intended termination as an operation is what makes the
absence of others meaningful. A model checker reports a state with no
enabled operation as a deadlock and cannot tell a machine that has
finished from one that is stuck, so before this schema existed the
deadlock check had nothing to say here --- and, because \texttt{spEnded}
was unreachable, nothing to say anywhere.

The reason this declares one halt rather than silencing the check is the
guard. \texttt{SessionOver} is enabled only where
\texttt{sessionPhase = spEnded}, so the only deadlock it can mask is the
one in that phase, which is the one intended. No other state's deadlock
is reachable through it. Every remaining deadlock in this specification
is therefore unintended by construction, and finding one is finding a
defect --- which is a stronger property than the document had before,
not a weaker one.
