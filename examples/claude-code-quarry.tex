...
All three toggles arrive the same way, as inputs to this operation, and
each is guarded or recorded by the term that names it. A nondeterministic
choice between two operations of equal precondition would also express
``something outside the machine decides,'' but it is the right form only
when the deciding datum cannot be named. Here it can: \texttt{sessionSync?}
is read from the same file, by the same hook, at the same moment as its
two siblings, and a specification in which two of three configuration
values are terms and the third is a choice of schema hides the symmetry
